...
function DingtalkDocPicker() {
  const toast = useToast();
  const { kbId, setSources, goToNext } = useDatasetImport();
  const [workspaces, setWorkspaces] = useState<{ id: string; name: string; description: string }[]>([]);
  const [workspaceId, setWorkspaceId] = useState("");
  const [nodes, setNodes] = useState<{ id: string; name: string; url: string }[]>([]);
  const [selected, setSelected] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);

  async function loadWorkspaces() { setLoading(true); try { const next = await api.listDingtalkWorkspaces(kbId); setWorkspaces(next); setWorkspaceId(""); setNodes([]); setSelected([]); toast(next.length ? `已读取 ${next.length} 个钉钉知识库` : "未找到可访问的钉钉知识库"); } catch (err) { toast(err instanceof Error ? err.message : "读取钉钉知识库失败"); } finally { setLoading(false); } }
  async function loadNodes(id: string) { setLoading(true); try { setWorkspaceId(id); setNodes(await api.listDingtalkNodes(kbId, id)); setSelected([]); } catch (err) { toast(err instanceof Error ? err.message : "读取钉钉文档失败"); } finally { setLoading(false); } }
  async function choose() { if (!workspaceId || !selected.length) return; setLoading(true); try { const picked = await Promise.all(selected.map(async (id) => { const node = nodes.find((item) => item.id === id)!; const preview = await api.previewDingtalkNode(kbId, id); return { id, createStatus: "waiting" as const, sourceName: preview.name || node.name, rawText: preview.text, connectorMeta: { source: "dingtalk", workspaceId, nodeName: node.name }, link: node.url }; })); setSources(picked); goToNext(); } catch (err) { toast(err instanceof Error ? err.message : "读取钉钉文档失败"); } finally { setLoading(false); } }
  return <Box className="feishu-import-panel"><Flex className="feishu-import-heading" align="flex-start" justify="space-between"><Box><Box className="feishu-import-title">选择钉钉文档</Box><Box className="feishu-import-desc">使用企业内部应用凭据和成员 User ID，读取该成员可访问的钉钉知识库与文档。</Box></Box></Flex><Flex className="feishu-folder-input" gap={3}><Box flex={1}><MySelect value={workspaceId} list={workspaces.map((workspace) => ({ value: workspace.id, label: workspace.name, description: workspace.description || undefined }))} onChange={(id) => void loadNodes(id)} placeholder={workspaces.length ? "选择钉钉知识库" : "请先读取知识库"} isDisabled={!workspaces.length} /></Box><Button onClick={() => void loadWorkspaces()} isLoading={loading} loadingText="读取中" variant="whiteBase">读取知识库</Button></Flex>{loading ? <Flex minH="150px" align="center" justify="center"><Spinner /></Flex> : nodes.length ? <Stack className="feishu-file-list" spacing={0}>{nodes.map((node) => <Checkbox key={node.id} className="feishu-file-row" isChecked={selected.includes(node.id)} onChange={() => setSelected((items) => items.includes(node.id) ? items.filter((item) => item !== node.id) : [...items, node.id])}><Box className="feishu-file-name">{node.name}</Box><Box className="feishu-file-kind">钉钉文档</Box></Checkbox>)}</Stack> : <Box className="feishu-import-empty">请先点击“读取知识库”。应用需开通知识库与企业存储文件读取权限。</Box>}<Flex className="feishu-import-footer" align="center" justify="space-between"><Box color="myGray.600" fontSize="sm">仅显示钉钉在线文档，导入后按当前处理配置自动切块和索引。</Box><Button onClick={() => void choose()} isDisabled={!selected.length} isLoading={loading}>下一步</Button></Flex></Box>;
}
